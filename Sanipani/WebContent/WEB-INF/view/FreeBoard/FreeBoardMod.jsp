...
function FreeBoardAdd(data, result){
	if(result =="success"){
		...
		$.ajax({
			type : "post",
			url : "updateFreeBoard", // TestController에서 마들기
			dataType : "json",
			data : params,
				...
				<input type="hidden" name="testNo" value="${param.testNo}"/>
					<div>
						<div class="freeboardword_2">
							제목:<input type="text" name="FreeBoardTitle" value="${con.FREE_TITLE}"/>
						</div>
					</div>
					<div class="freeboardword_2">내용
					</div>
					<div class="freeboardword_3">
						<textarea cols="94" rows="33" style="resize: none;" name="FreeBoardContent">${con.FREE_CONTENT }</textarea>
					</div>
