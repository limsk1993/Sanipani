...
$(document).ready(function(){
	$(".AgreeBtn").on("click",function(){
		if(confirm("계속진행하시겠습니까???????")){
			...
			$.ajax({
				type : "post",
				url : "EscrowUpdateAuction", // TestController에서 마들기
				dataType : "json",
				data : params,
				success : function(result){
				
						
						$("#EscrowInsert").attr("action", "AuctionRequest1");
						$("#EscrowInsert").submit();
		...
		<div class="content">
				<div class="EscrowContent">
					...
					<form action="#" method="post" id="EscrowInsert">
						...
						<input type="hidden" name="Trade_Status_No" value="7">
						<input type="hidden" name="Member_No" value="${sNo}">
						<input type="text" name="auctionNo" value="${param.auctionNo }"/>
