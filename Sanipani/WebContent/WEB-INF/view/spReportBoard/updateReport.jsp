...
$(document).ready(function() {
	...
	$("#saveBtn").on("click", function(){
		var updateRepForm = $("#updateRepForm");
		
		updateRepForm.ajaxForm(uploadResultCallBack);//ajax를 실행하고나서 uploadResultCallBack라는 함수를 호출하겠다.
		updateRepForm.submit(); // 폼 자체가 실행할때 ajax로 구동을 하겠다. 기존엔 ajax로 이동한것.
...
function uploadResultCallBack(data, result){ 
	if(result == "success"){
		...
		//$("#testFile").val(resData.fileName[0]);//textFile에 업로드된 파일네임 갖다넣음
	
		var params = $("#updateRepForm").serialize(); //파일업로드를 하고난 후 글 저장을 하겠다.
	...
	<div class="ReportCategory">
		...
		<form action="fileUploadAjax" 
			  ...
			  enctype="multipart/form-data" text-align="center">
				...
				가해자 닉네임 <input type="text" class="FraudNick" name="FraudNick" /> <br /> 
				물품 <input type="text" class="ProductName" name="ProductName" /> <br /> 
				내용<textarea rows="10" cols="80" name="Reportcontent" style="margin: 0px; width: 619px; height: 195px;"></textarea> <br />
